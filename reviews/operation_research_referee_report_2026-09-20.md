...
Unfortunately, the central scientific claims do not survive close scrutiny in their present form. The principal Operations Research structure theorem has a sign problem that produces the opposite comparative static in the simplest admissible special case. A central “non-equivalence” result for entropic risk-sensitive control is mathematically false as stated because the entropic Bellman term has a standard variational representation as a supremum of affine continuation-value functionals penalized by relative entropy. The core inventory experiments do not instantiate the inventory valuation model stated in the theory: in the main simulator, theta acts primarily as an extra policy gate entering the order-up-to formula, while the shortage cost remains fixed. The headline inventory advantage also benefits from a larger optimization budget; the equal-budget rerun reverses the welfare and cost ordering, and the later positive 1536-evaluation result follows configuration screening and therefore is not an independent confirmation.
...
2. a policy-class enlargement from a to (a, theta);
...
Theorem thm:inventory_service_pressure_structure is presented as the main OR structural result. The representative one-period objective in main.tex (subsection “Endogenous Service-Pressure Structure in Inventory”) contains, inside a maximization problem,

    - p(theta) E[(B + D_z - S)^+] - lambda (theta - theta^-)^2,

with p'(theta) > 0.

The theorem then claims that the optimal valuation/service-pressure selector theta* is nondecreasing in backlog B and demand stress z. The proof states that increasing differences hold because p(theta) multiplies unmet demand. This is backwards: the manuscript has minus p(theta) times unmet demand.

A one-period counterexample is immediate. Set continuation value to zero, let p(theta) = theta, and for the moment hold S fixed. Define

    H(B,z,S) = E[(B + D_z - S)^+] > 0.

The theta-dependent objective is

    - theta H(B,z,S) - lambda (theta - theta^-)^2.

When the interior solution is feasible,

    theta* = theta^- - H(B,z,S) / (2 lambda).

Hence theta* is decreasing, not increasing, in expected shortage and therefore generically decreasing in backlog/stress. The same sign problem remains relevant when S is jointly optimized; the stated assumptions do not provide a continuation-value cross-effect strong enough to reverse it.

This is not a cosmetic proof gap. It exposes a conceptual problem: if theta is literally an internally chosen penalty on shortage, a decision maker maximizing its own objective can improve that objective by lowering the penalty when shortage becomes painful. That is preference manipulation/reward hacking, not an operational service-pressure response.

### Required correction

The authors need to decide what theta physically is.

- If theta is an external service commitment, contractual penalty, inspection intensity, safety margin, or resource-backed target that changes the feasible system or creates an external transfer, model that mechanism explicitly.
- If theta is merely an internal weight in the decision maker’s own objective, then the direction of optimal adjustment must be derived honestly, and the interpretation “higher stress increases service pressure” cannot be asserted without additional structure.
...
Second, and more decisively, the standard Gibbs/Donsker–Varadhan variational identity gives, for a finite nominal transition distribution p and rho > 0,

    (1/rho) log sum_i p_i exp(rho V_i)
      = sup_{q in Delta} { sum_i q_i V_i - (1/rho) D_KL(q || p) }.

The right-hand side is exactly a supremum of affine continuation-value functionals with a theta = q dependent intercept/penalty. In other words, an entropic risk-sensitive operator has precisely the kind of variational support representation the manuscript says cannot coincide with NDU except when affine.
...
Delete the current broad non-equivalence claim. At most, prove a narrowly defined non-equivalence for a specific restricted parameterization of g_theta and P_theta that excludes the variational representations used by risk-sensitive/robust control. The paper must distinguish “our chosen parameterization differs from class X” from “class X cannot represent our Bellman operator.”

## 5. The “generalization of reinforcement learning” result is mostly an action-space enlargement

The manuscript correctly admits that, once (c,u) is treated as the state and (a,theta) as the control, NDU is an augmented-state stochastic-control problem. That admission is important, but it also weakens the title-level theorem substantially.

The frozen-face embedding is essentially an embedding from A into A × Theta by fixing theta = theta_bar. A larger action set weakly improves the optimum; a strictly useful extra action can strictly improve it. This is not a new expressivity theorem about dynamic programming or reinforcement learning.

The operator-level theorem does establish a small mathematical fact: after optimizing theta out, the resulting action-conditioned term need not be one fixed affine functional of continuation value. But an ordinary MDP/control problem with the expanded action (a,theta) represents the model exactly. Therefore the “non-collapse” result is largely a consequence of insisting that the comparator retain the old physical action set A while NDU receives A × Theta.

For *Operations Research*, the important contribution would have to be why theta is an economically/operationally legitimate scarce control and what new policy structure or performance consequences follow from that fact—not the observation that adding a control coordinate enlarges a policy class.

### Required correction

Reposition the contribution as an endogenous service-standard/valuation-control model, not a generalization of RL in an expressivity sense. State explicitly that standard augmented-state RL/control can solve the model when theta is included as an action. Then identify the genuinely new OR object (for example, commitment cost, service-level adjustment friction, or endogenous risk budget) and derive nontrivial structure from that object.
...
- Full NDU computes theta = 0.55 + 1.65 * sigmoid(...);
- that theta enters the policy formula for order_up_to;
- the shortage cost is fixed as shortage_cost = 2.85 * backlog;
- the only direct objective term involving theta is theta_cost = 0.045 * (theta - 1.0)^2.

Thus the main experimental theta is primarily an additional nonlinear policy gate/action parameter. It is not the theoretical p(theta) shortage-penalty coefficient in equation eq:inventory_service_pressure_objective.

The exact finite-cover/policy-class scripts use yet another construction: theta enters a service/fill reward multiplier such as 8.0 * theta * (fill_rate - 0.92), again not the theoretical -p(theta) × shortage term. The SKU exact MDP uses still another calibrated valuation benefit/service penalty.
...
Theorem thm:stable_physical_cost_improvement assumes state/regime-specific physical shortage costs p_z, then lets the NDU service-pressure rule choose theta_z satisfying p(theta_z) = p_z. This produces the regime-specific newsvendor critical fractile S_z*. The comparator is forced to use one fixed p_bar.
...
The theorem does not show that the NDU objective learns or optimizes toward p(theta_z) = p_z. It simply gives NDU the correct regime-specific physical penalty by assumption. A same-information action-only policy that directly optimizes the theta-free physical cost can also choose the regime-specific S_z* without introducing a preference-control variable at all.
...
Either derive theta_z = p_z (or an approximation to it) from the actual endogenous-theta optimization problem, or present this result honestly as a calibration lemma showing the cost of a misspecified fixed service-pressure parameter. It should not be one of the main proofs of NDU’s practical value.

## 8. The compact-domain HJB certificate theorem is incomplete at the spatial boundary

Theorem thm:nbo_hjb_solver_certificate works on a compact parabolic domain D = [0,T] × K and states a sup-norm value bound using an interior HJB residual and terminal residual. The theorem statement does not specify data on the nonterminal spatial boundary [0,T) × boundary(K), nor does it define a state-constraint/reflected boundary problem.
...
Corollary cor:cover_refinement_hjb_certificate says that monotone, stable, consistent state-time covers plus interpolation error eta_h and action fill distance rho give a policy bound of the form
...
- native HJB loss 6.26 × 10^4;
- normalized residual RMSE 1.84;
- terminal loss 2.06 × 10^3.

The sampled greedy-gap audit is also very poor:

- actor residual L2/sup approximately 204.6 / 805.6;
- grid-optimal residual L2/sup approximately 417.0 / 1315.9;
- greedy gap p95/sup approximately 1257.7 / 1556.6;
- terminal sup approximately 75.1.

The actor-gap-focused inventory run reduces the greedy-gap sup to about 120, but terminal error remains about 75. The residual-focused run drives other terms down while the greedy-gap sup grows to roughly 1652. The selected normalized-residual Pareto run still has normalized RMSE approximately 1.805, meaning the residual remains of the same order as or larger than the Hamiltonian scale under the manuscript’s own interpretation.
...
The canonical script reproducibility/scripts/run_inventory_service_level_benchmark.py allocates different CEM budgets:

- Full NDU: 11 × 26 = 286 policy evaluations;
- action-only adaptive base-stock: 9 × 22 = 198;
- fixed service: 8 × 18 = 144.
...
- On the public SKU panel, Approximate DP has lower external cost (about 0.3537 versus 0.3558), higher fill, and lower backlog than Full NDU; Full wins only on its valuation-adjusted welfare.
- In the exact policy-class DP, Full has higher valuation welfare but worse average cost (about 3.3912 versus 3.1948 for action-only).
- In the exact valuation-state MDP, Full improves valuation welfare but has the same theta-free cost/fill as action-only.
- In the PyTorch inventory row, Full improves the internal service-pressure welfare but has worse reported cost than action-only (about 5.0211 versus 4.9314).
...
The manuscript should not let a theta-dependent welfare improvement stand in for operational superiority.
...
The algebraic lemma is correct under its assumptions:
    ...
    Z_u = sqrt(2 epsilon) P_u,

and additive Z-target noise of variance independent of epsilon produces P-error amplified by 1/(2 epsilon).
...
The empirical direct-P/Z experiments largely build in the same equal-scale perturbation, so they are primarily demonstrations of the algebra. In the same-architecture neural ablation, learned-Z even has better welfare at epsilon = 10^-1 and 10^-2, becoming worse only in the near-singular 10^-3 regime.
...
Use a realistic end-to-end learning model in which noise/approximation error arises from the training procedure, report how error scales with epsilon, and narrow the wording from “paradox”/solver superiority to a conditioning result.
...
4. **Comparator semantics.** “Fixed preference,” “action-only,” “preference backbone,” and “augmented fixed-theta” should be defined once, with matched information, parameter count, policy architecture, and optimization budget.

5. **State/action semantics.** If theta changes the policy map directly, it is an action/control coordinate. If it changes a utility/penalty, it is a preference/evaluator coordinate. If it changes a service contract, it is an operational state/control with external consequences. The paper currently moves among all three meanings.
...
Give theta a single operational meaning with external consequences or a defensible commitment/resource interpretation. Do not let the decision maker simply choose the coefficient that grades its own shortage.
...
The exact DP, finite-cover problem, simulator, and theorem must use the same p(theta), adjustment cost, transition law, and state variables. Any neural policy architecture should be an approximation layer on top of that model, not a new definition of theta.
