...
\begin{document}
\begin{center}
{\Large\bfseries Electronic Companion\par}
\medskip
{\large Neural Differential Utility: Accepted Dynamic Service Contracts and Certified Value-Gradient Learning\par}
\medskip
{Revision R6, September 21, 2026}
\end{center}
This companion preserves the earlier valid supporting derivations and full numerical baseline. Sections EC.1--EC.5 retain comparator geometry, physical metrics, conditioning, constructive critics, and historical evidence. The formerly separate general diffusion theorem and manufactured checks follow in EC.6--EC.7. They are not substituted for the new nonmanufactured operational learning problem. Current root manuscript, companion, response, and revision directory all identify R6. Original R4 scientific sources and PDFs remain unchanged in the pre-R6 archive; the earlier pre-R4 history is also intact.
\section{Comparator geometry and inner optimization}
...
\end{proposition}
\begin{proof}
On $U_i$, the derivative of $F$ is $\beta p_{j_i}$. Equality with a single affine function on an open set would require its derivative to equal both distinct vectors. This is impossible. The last statement follows by taking the ordinary action set to be the original physical action paired with $j$.\end{proof}
...
\[
 Z=\sum_i p_i e^{\rho\beta v_i},\qquad
 q_i^*=p_i e^{\rho\beta v_i}/Z.
\]
...
\[
 \nabla f(\widehat\theta)=n+e,\quad
 n\in N_\Theta(\widehat\theta),\quad\norm e\leq\varepsilon,
\]
...
\[
 f(\theta^*)-f(\widehat\theta)
 \leq\varepsilon d-\mu d^2/2\leq\varepsilon^2/(2\mu),
 \quad d=\norm{\theta^*-\widehat\theta}.
\]
For the distance bound, combine the strong monotonicity of $-\nabla f$ with the first-order optimality condition at $\theta^*$ and the stated approximate normal-cone condition. They imply $\mu d^2\leq\varepsilon d$. Completing the square in the quadratic objective proves the projection statement.\end{proof}
...
\section{Physical metrics and the historical newsvendor argument}
...
\begin{equation}
 C_z(\bar S_z)-C_z(S_z^*)
 \geq\frac{(h+p_z)\underline f_z}{2}(\bar S_z-S_z^*)^2.
\end{equation}
...
\[
 W^J-W^F=0.944747060878850\ldots,
 \qquad C^F-C^J=0.213203951173630\ldots .
\]
...
\section{Continuous-time derivations and boundary controls}
...
\[
 w(t,x)=(1-t)^{-1/2}\exp\{-x^2/[4(1-t)]\},\quad t<1,
 \qquad w(1,x)=0.
\]
...
The main theorem separately bounds value error and the critic-to-actor value error. For an actor-induced residual, the difference between the actor and optimal Hamiltonians must also be charged. A Neumann defect, reflected boundary local time, and a state-constraint viscosity inequality are not interchangeable with the Dirichlet mismatch used here.
...
\clearpage
\section{R6 implementation and independent checking}
The current experiments are in \texttt{revisions/or-r6-20260921/}. \texttt{compute.py} enumerates all continuous-static candidates as rational numbers, solves ordinary dynamic programs and the full-stock occupation programs, and implements the time--regime policy restriction with shared binary tier selectors. \texttt{exact\_certificate.py} independently reconstructs the two deterministic protocols underlying the customer-preserving mixture, propagates their laws using rational arithmetic, and checks all three commitment equalities and a Lagrange Bellman upper bound. Its proof does not use a linear solver.

The time--regime mixed-integer program uses 2,904 stock-reduced occupancy variables and 264 binary tier selectors. For each $(t,z)$, the selectors sum to one; $x_t(z,q,\theta)\leq y_{t,z,\theta}$ for every $q$. Occupancies never exceed one, so this bound is valid. Unreachable states may have arbitrary selectors. The reported global gap is a numerical solver certificate, not an exact rational branch-and-bound proof. The inherited-only value is proved equal to the time-only value, not estimated from a timed-out integer solve.

\texttt{envelopes.py} compares full backward induction using ordered hulls and pairwise enumeration for quadratic and quartic-potential adjustment. It records both candidate counts and measured times. \texttt{continuous\_learning.py} implements the explicit controlled chain, the nonmanufactured refinement, paired value-only and value-gradient training, saved weights, projected derivative actors, policy evaluation, and all-state residual budgets. Training uses float32; subsequent chain policy and residual recursions use float64. The 1,800-step pilot is retained separately and is not pooled with the 6,000-step comparison. Wall times are measured on one CPU environment, not portability guarantees.

The exact discrete-contract results, numerical MIP and LP diagnostics, nonmanufactured chain refinement, and trained-network evaluations are separate evidence classes. In particular, a numerical finite-chain residual budget is not an interval-arithmetic enclosure of a continuum HJB residual. The companion retains the hypotheses needed for such a continuum verification result, including the boundary mechanism. No private customer data, price calibration, hidden-effort identification, or general high-dimensional learning advantage is asserted by this synthetic study.
