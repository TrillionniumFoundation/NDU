...
mkdir -p "$R/results"
python "$R/analyze.py" > "$R/results/analysis.log"
# Stable external-label copies avoid cross-document recursive aux imports.
for i in 1 2 3 4; do
  pdflatex -interaction=nonstopmode -halt-on-error main.tex > "$R/results/main-build-pass-$i.txt"
  grep '^\\newlabel' main.aux > r25-main-labels.aux
  pdflatex -interaction=nonstopmode -halt-on-error electronic_companion.tex > "$R/results/ec-build-pass-$i.txt"
  grep '^\\newlabel' electronic_companion.aux > r25-ec-labels.aux
done
