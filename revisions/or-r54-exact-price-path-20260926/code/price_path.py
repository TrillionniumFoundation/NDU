...
def solve(spec,epsilon=F(0),price_steps=12,max_nodes=255,seconds=None):
    ...
    tree={'proof':{'type':'universal','upper':str(d.mean_reward(F(1)))}};trace=[];evaluated=0
    ...
    def snapshot():
        trace.append(dict(seconds=time.perf_counter()-start,lower=str(incumbent['value']),upper=str(upper()),
    ...
    try:
        ...
        while evaluated<max_nodes:
            ...
            for step in range(price_steps):
                ...
                chosen=ans
                if ans['upper']<F(node['proof']['upper']):node['proof']={'type':'price','price':str(lam),'upper':str(ans['upper'])}
                policy=allocate(d,ans['book'],B,dict(zip(a,rho)))
                if policy['value']>incumbent['value']:incumbent=policy
                snapshot()
            ...
            if node['proof']['type']=='infeasible' or F(node['proof']['upper'])<=incumbent['value']+eps:continue
            checktime()
            ...
            if len(required)==m or not optional:
                ...
                node['proof']={'type':'price','price':str(pol['price']),'upper':str(pol['value'])}
    ...
    snapshot();U=upper()
    cert=dict(schema='NDU-price-path-v1',spec=spec_for(d,a,rho,B,spec['budget']),epsilon=str(eps),tree=tree,
              policy=encode(incumbent),lower=str(incumbent['value']),upper=str(U))
