...
def verify(cert,expected_sha256=None):
    begin=time.perf_counter();require(cert['schema']=='NDU-price-path-v1','Unsupported certificate')
    ...
    return dict(status='PASS',lower=str(lower),upper=str(upper),gap=str(upper-lower),
...
def verify_common_cap(cert,expected_sha256=None):
    ...
    return dict(status='PASS',books=len(seen),lower=str(lower),upper=str(upper),seconds=time.perf_counter()-begin)
