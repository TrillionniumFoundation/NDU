...
BASE='1cf9c8df76437ee71d4291d1001e9c7f055781c5'
BRANCH='revision/ndu-operations-research-r38-contractual-quantization-20260924'
B=ROOT/'.build/ndu-r38'; B.mkdir(parents=True,exist_ok=True)
...
def preservation():
    ...
    result={'base_review_sha':BASE,'new_branch':BRANCH,'all_prior_revision_directories':'unchanged',
            'predecessor_readers':{}}
    if (ROOT/'.git').exists():
...
def prepare():
    preservation()
    for script in ('verify.py','study.py','inherited.py','prepare.py'):
...
def compile_one(name):
    with (B/(Path(name).stem+'-console.log')).open('w') as log:
        run(['pdflatex','-interaction=nonstopmode','-halt-on-error','-file-line-error',
             '-output-directory='+str(B),name],stdout=log,stderr=subprocess.STDOUT)
...
def build():
    ...
    response_tex()
    for _ in range(4):
        compile_one('main.tex'); labels('main','r38-main-labels.aux')
        compile_one('electronic_companion.tex'); labels('electronic_companion','r38-ec-labels.aux')
    ...
    assert nonreference<=30,('Regular-paper nonreference limit',nonreference)
    ...
    for name in ('verification.json','catalog_study.json','inherited.json'):
        assert json.loads((R/'results'/name).read_text())['status']=='PASS'
    manifest=dict(status='PASS',utc=datetime.now(timezone.utc).isoformat(),base_review_sha=BASE,
                  source_commit=git('rev-parse','HEAD') if (ROOT/'.git').exists() else 'local-artifact-worktree',
...
if __name__=='__main__':
    if len(sys.argv)!=2 or sys.argv[1] not in ('prepare','build'):
        raise SystemExit('Usage: build.py prepare|build')
    globals()[sys.argv[1]]()
