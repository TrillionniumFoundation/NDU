...
B=ROOT/'.build/ndu-r42'; B.mkdir(parents=True,exist_ok=True)
...
def prepare():
    preservation()
    for script in ('verify.py','study.py','inherited.py','tables.py'):
...
def build():
    ...
    response_tex()
    for _ in range(4):
        compile_one('main.tex'); labels('main','r42-main-labels.aux')
        compile_one('electronic_companion.tex'); labels('electronic_companion','r42-ec-labels.aux')
    ...
    for name in ('verification.json','inherited.json'):
        assert json.loads((R/'results'/name).read_text())['status'].upper() in ('PASS','PASSED')
