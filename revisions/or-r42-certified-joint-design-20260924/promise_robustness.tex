...
\end{equation}
This is a sufficient certified interval of strict advantage, not a maximal interval or an exact nonsaturated loss formula. The global cell algorithm below computes exact values outside this conservative interval as well. At the other endpoint, $B\leq1/4$ gives zero loss with one symbol. In between, cap anchoring at the original $b_j$ and pathwise--deterministic equality do not follow from the saturated proofs. Indeed the one-symbol full optimum $C=B<\min b_j$ already shows why original-cap anchoring cannot hold for all promises. The decision insight persists without implying that the saturated algorithm solves every interior promise.
...
Theorem~\ref{thm:eligible} extends the fixed-book response to every realization ceiling. Theorems~\ref{thm:continuous} and~\ref{thm:mesh} solve the continuous outer design and certify its catalog approximation; thus the inner allocation is no longer the endpoint of the nonsaturated analysis.
