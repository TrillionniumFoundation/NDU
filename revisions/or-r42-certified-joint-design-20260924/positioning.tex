...
\]
It is generally not a common squared-distance projection penalty: it carries a level-dependent linear term and a heterogeneous intermediate-service curvature. The paper's combined saturated structural result concerns this one-sided contractual tail, its continuously chosen endpoint, and the cancellation that preserves the terminal Monge order. The affine movement of an interior codeword follows the quadratic interpolation structure and is not advertised as an independent general quantization principle. Its sufficient conditions are explicit: saturated branch targets, quadratic terminal reward, uncharged codewords, and intermediate costs arising only above the highest level. Section~\ref{sec:promise} treats endogenous targets, and Section~\ref{sec:catalog} treats actual level charges without assuming the same anchoring lemma.

\paragraph{Optimized dual grids and enabled-level penalties.}
\citet{JourdainPages2021} prove uniqueness of optimal one-dimensional dual grids for compactly supported log-concave densities and develop a Lloyd-like algorithm in the quadratic case. That result directly concerns optimized grids, not merely evaluation of a prescribed rounding rule. Here branch targets are themselves constrained decisions, intermediate shortfall costs can differ by branch, and each branch admits a different eligible prefix. Our exact continuous reduction does not assume a log-concave source density or uniqueness of a stationary grid; the grid approximation also preserves the contractual constraints at their boundary.

A charge for enabling a representative is a fixed-opening-cost idea, not a new type of resource pricing. For example, \citet{AardalEtAl2014} distinguish facility-opening costs from demand-dependent service costs under a cardinality restriction. In our model, a selected level can implement adjacent random splitting and branch-specific intermediate shortfalls, rather than serving a client by ordinary distance to one center. The opening charge is not entropy, expected message length, or a cost per token use. The new issue is its joint optimization with exact promises and realization eligibility. The finite quadratic search in Theorem~\ref{thm:continuous} invokes a classical optimization principle; its contribution is the contractual reduction to those closed cells. Theorem~\ref{thm:mesh} gives the separate, constraint-preserving approximation guarantee.
