...
An exact algorithm enumerates at most $(2s-1)^k$ cells at size $s$. After eliminating one target, each cell has dimension at most $s+k-1$ and at most $s+1+3k$ inequalities. Enumerating its independent active sets and applying Lemma~\ref{lem:faces} gives a finite global algorithm with at most
\begin{equation}\label{eq:face-complexity}
 \sum_{s=1}^m (2s-1)^k\,2^{s+1+3k}
\end{equation}
linear systems, each of polynomial dimension and rational encoding length in the input size. This is an exponential reference algorithm, not a polynomial-time claim or an extension of the saturated Monge complexity.
\end{theorem}
\begin{proof}
