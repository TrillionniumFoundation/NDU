...
R=Path(__file__).resolve().parents[1];ROOT=R.parents[1];B=ROOT/'.build/r47'
...
def build():
    ...
    for x in ['main.aux','electronic_companion.aux']:
        f=B/x
        if f.exists():labels(f,ROOT/('r47-main-labels.aux' if x=='main.aux' else 'r47-ec-labels.aux'))
    for _ in range(3):
        tex('main.tex');labels(B/'main.aux',ROOT/'r47-main-labels.aux')
        tex('electronic_companion.tex');labels(B/'electronic_companion.aux',ROOT/'r47-ec-labels.aux')
    ...
    stems=['main','electronic_companion']+(['RESPONSE_TO_REFEREES'] if (R/'RESPONSE_TO_REFEREES.tex').exists() else [])
    logs='\n'.join((B/(n+'.log')).read_text() for n in stems)
    ...
    for f in (R.parent/'or-r46-box-decomposition-20260925').glob('*.tex'):
        if 'RESPONSE' not in f.name:
            oldlabels+=re.findall(r'\\begin\{(?:theorem|proposition|lemma|corollary)\}(?:\[[^\]]*\])?\s*\\label\{([^}]+)\}',f.read_text())
    ...
    newlabels=set(re.findall(r'\\begin\{(?:theorem|proposition|lemma|corollary)\}(?:\[[^\]]*\])?\s*\\label\{([^}]+)\}',(R/'coarsening.tex').read_text()))
    if len(newlabels)!=4 or newlabels-labelsnow:raise RuntimeError(('New mathematical statement labels',newlabels-labelsnow))
    intro=(R/'introduction.tex').read_text()
    if '$' in intro or any(x in intro for x in ('\\[','\\(','\\begin{equation','\\begin{align')):raise RuntimeError('Introduction contains mathematical notation')
    if len(ABSTRACT.split())>200:raise RuntimeError('Abstract exceeds OR limit')
    inputs={}
    ...
    git=lambda *args:subprocess.check_output(['git',*args],cwd=ROOT,text=True,stderr=subprocess.DEVNULL).strip()
