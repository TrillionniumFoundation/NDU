...
\begin{document}
\begin{center}\Large\bfseries Response to the Referee: Revision R36\end{center}

\textbf{Manuscript:} Accepted Service Adaptation: Exact Parametric Quotients and Minimal Additional Writable Memory.


\textbf{Report addressed:} the independent Operations Research report at \nolinkurl{a017f474619e86be533547acac87a3c8354f64cf}, reviewing R30 at \nolinkurl{fdc1ed47d46dc9d3344b8bce47c1670f11dcd475}. \textbf{Scientific baseline:} the complete R35 reader at \nolinkurl{7e49851cd04f0f7e015c2561b43a4e7591574e67}. \textbf{New branch:} \nolinkurl{revision/ndu-operations-research-r36-linear-frontier-20260924}. September 24, 2026.
...
\section*{1. Version identity and the scientific change}

The report's version-audit concern is addressed by immutable identities rather than branch names alone. The baseline, report, and scientific source are separately recorded in \nolinkurl{DERIVATION_PROVENANCE.md} and the machine-readable build record. The four original reader/entry wrappers are archived under \nolinkurl{predecessor/}. Every inherited theorem module remains unchanged, and a full remote base-tree audit rejects deletion or unrelated modification.


The new result is Theorem \nolinkurl{thm:r36-linear}. It computes each complete expected-participation or deterministic/pathwise memory frontier through budget m on k ordered branches in O(mk) exact rational operations and comparisons, with O(mk+k) stored scalars and indices. It includes one optimal codebook per budget and exact continuous optimization of the highest randomized level. The preceding R34 construction required quadratic work per layer; R35 proved and implemented logarithmic-factor monotone search. R36 proves the missing all-submatrices property, implements classical SMAWK, and retains both preceding algorithms as exact same-input comparators.


\section*{2. Parametric optimization and the residual priority question}
...
\section*{4. Classical machine minimization and the accounting convention}
...
\section*{5. Finite contract policy graphs and operational interpretation}
...
\section*{6. Randomization, global design, and participation}
...
\section*{7. Comparative statics and the scope of dispersion}
...
\section*{8. Bit complexity, comparisons, and intermediate arithmetic}
...
The new theorem is likewise explicit. On ordered inputs, linear search performs O(mk) rational operations and comparisons. The moment and continuous-terminal expressions are unchanged, so the existing coefficient-height bound S gives the conservative O(mkS$^3$) Turing bound. Symbolic keys require only an order tag and a column index in addition to a finite exact cost. Sorting unordered caps adds its own O(k log(k+1)) rational comparisons. Winner re-queries, terminal scans, backpointers, and all-budget reconstruction are counted. No quadratic table or unlimited memoization cache is concealed in the linear-work claim.


\section*{9. Computational alignment, independent evidence, and negative results}
...
\section*{10. Supporting theory, presentation, and review readiness}
