...
\begin{equation}\label{eq:unified-feasible}
 ...
 \sum_j\pi_jt_j=B.
\end{equation}
A book $c$ has at most $m$ distinct levels in $[0,1]$ and incurs $\sum_{z\in c}\rho(z)$, once per selected level. Let $\mathcal V_m^\delta(B;\rho)$ denote maximum operating payoff less this charge. A finite catalog restriction is denoted $\mathcal V_{m,\mathcal A}^\delta(B;\rho)$. The expectation cap is not replaced by the weaker realization ceiling. We write $V_m^P(B)$ and $V_m^E(B)$ for the same zero-charge objective under, respectively, pathwise participation and expectation-only participation, with their encoder restrictions unchanged. Taking $\delta\geq1$ makes the realization restriction redundant at the canonical optimum below. Taking $\delta=0$ gives the full pathwise value, even though that institution initially permits draw-specific intermediate service. Thus these are endpoints of one design problem, not three unrelated extensions.
...
For every exact-arithmetic and bit-complexity statement we assume rationally encoded probabilities, caps, ceilings, catalog levels, promise, and nonnegative charges, with
\begin{equation}\label{eq:r34-primitives}
 f(x)=rx-\frac q2 x^2,\quad r>0,\quad 0\leq q\leq r,\qquad
 h_j(y)=\frac{\gamma_j}{2}y^2,\quad\gamma_j\geq0.
\end{equation}
The structural identities apply more generally under their stated concavity assumptions; exact bit complexity does not refer to arbitrary real inputs. For these quadratic primitives, chord $[u,v]$ has slope $r-q(u+v)/2$. On branch $j$ its target capacity is
\[
