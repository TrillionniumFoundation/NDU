...
Define the catalog signature $K_j(\mathcal A)=|\mathcal A\cap[0,\tau_j]|$. Two histories have the same \emph{eligibility class} when their signatures coincide. This partition depends on the candidate catalog, not only on the primitive contracts. Let $G_1,\ldots,G_E$ be these classes, and write
\[
...
\end{theorem}
\begin{proof}
Use the canonical response in Theorem~\ref{thm:eligible}. Suppose a feasible allocation gives positive intermediate service to one member while another has unused terminal capacity below $\min\{b_j,d_g\}$. If member $i$ has positive service and member $j$ has spare terminal capacity, choose $\delta>0$ small enough that both capacity inequalities remain valid and set $t_i^{\prime}=t_i-\delta$, $t_j^{\prime}=t_j+(\pi_i/\pi_j)\delta$. The conservation equation $\pi_i(t_i^{\prime}-t_i)+\pi_j(t_j^{\prime}-t_j)=0$ keeps the class mean unchanged. Intermediate cost cannot increase, and the strictly increasing common interpolant raises terminal reward. Thus no optimum mixes positive service with unfinished terminal capacity.
...
If $s\geq D_g(d_g)$, every terminal capacity is full and only service remains to be allocated. Its capacities and total are precisely those in \eqref{eq:r49-C}, proving the second case. At $s=D_g(d_g)$, both formulas equal $\sum_jw_{j|g}F_{g,c}(\min\{b_j,d_g\})$, because $C_g(0;d_g)=0$; continuity of the finite capped convex allocation value gives continuity at this boundary. If $s>D_g(d_g)$ and the equalization level were at most $d_g$, its weighted mean would be at most $D_g(d_g)$, a contradiction. Hence $\theta_j^g(s)\geq\min\{b_j,d_g\}$, so the equalized allocation has the same terminal rewards as the optimal allocation. Their difference is exactly the reduction in service cost in \eqref{eq:r49-delta}. Below $D_g(d_g)$ both cost terms are zero. Feasibility follows from the original cap constraints and the canonical adjacent lottery or singleton, including its supportwise ceiling.\end{proof}
...
The corrected path recurrence returns the globally best book and individual targets for supplied feasible group means, at every budget up to $m$. For rationally encoded nonnegative opening charges and rational quadratic primitives $f(x)=rx-qx^2/2$ and $h_j(y)=\gamma_jy^2/2$, including zero service curvatures, one call takes
\begin{equation}\label{eq:r49-oracle-cost}
 O\bigl((k+m)N^2+kN\log(k+1)\bigr)
\end{equation}
rational arithmetic operations, with polynomial bit complexity in the instance and supplied means. When $E=1$, set $s_1=B$: a single call solves the original joint finite-catalog problem globally, for variable $k,N,m$, nonnegative opening charges, and heterogeneous caps and curvatures. Optimization identities also permit real charges; the bit-complexity assertion requires the rational encoding just specified. It does not enumerate books or expand the command budget.
\end{theorem}
\begin{proof}
