# R58 claim, response, and preservation map
...
|---|---|---|
| R56 absent revision, misidentified reader | Current root readers, R58 response introduction | Actual recorder input graph, source/PDF hashes, publication and preservation audits |
...
3. The recovered local R56 development package has an explicitly recorded SHA-256. It is a development input, not evidence of earlier remote publication, and its timings are not pooled with R58.
4. R58 supplies a new scientific delta and freshly executes the disclosed 225-run design. Separate labels distinguish primary, extension, untimed mechanics and extended verification. Earlier post-inspection protocol text is retained under `development/recovered_protocols/` for provenance; it is not an R58 preregistration claim.
5. The different absent R55 local 269-run archive is not fabricated or counted as current evidence.

The initial R58 checker refactor is semantic-preserving: enumeration and screening proof functions are moved to verifier-only modules. The import-graph regression verifies their separation from the optimization modules. The new joint-type constructor is a scientific implementation addition, not a renamed common-reward check.
