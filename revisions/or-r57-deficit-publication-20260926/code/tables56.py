...
def main():
    ...
    ec.extend([r'\begin{figure}[p]\centering',r'\includegraphics[width=\textwidth]{revisions/or-r57-deficit-publication-20260926/generated/exact_trajectories.pdf}',r'\caption{Complete recorded exact-target price trajectories for the two eight-history instances requiring branching. Every recorded epoch is retained. The final circles denote zero rational width; elapsed times are those of the published two-second-limit runs, not the later mechanical replay.}\label{fig:trajectory56}',r'\end{figure}\clearpage'])
