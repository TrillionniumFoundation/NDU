"""Assemble, compile and validate the actual R57 reader graph from repo root."""
from pathlib import Path
import datetime,hashlib,json,re,shutil,subprocess,sys,platform
import fitz
R=Path(__file__).resolve().parents[1];ROOT=R.parents[1];OUT=ROOT/'.build/r57'
...
def assemble():
    ...
    ext=lambda name:'\\externaldocument{.build/r57/'+name+'}['+name+'.pdf]\n'
...
\vspace{0.25in}Anonymous manuscript for Operations Research --- R57\par\vspace{0.2in}
...
'''
    sections=['introduction','model','constructive','@revisions/or-r53-catalog-safe-certificates-20260926/capacity_potential','exact_caps','hardness','small_menus','price_duality','packing','branching','uniform_summary','deficit_paths','lattice','study','new_study','conclusion']
...
'''
    ec+=inp(f'{REL}/diagnostics56.tex')+inp(f'{REL}/operations_interface.tex')
...
'''
    ...
    response=pre+ext('main')+ext('electronic_companion')+r'\begin{document}\begin{center}{\Large\bfseries Response to the R56, R55, and R54 Referee Reports}\par '+TITLE+r'\par R57 --- September 26, 2026\end{center}'+'\n'+inp(f'{REL}/response_body.tex')+'\\end{document}\n'
