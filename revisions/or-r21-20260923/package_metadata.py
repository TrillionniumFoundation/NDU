...
R=Path(__file__).resolve().parent;ROOT=R.parents[1];P='revisions/or-r21-20260923'
...
def run():
 ...
 intro=f'''# Neural Differential Utility — Operations Research R21

**{title}**

Revision: September 23, 2026. Branch: `revision/ndu-operations-research-r21-20260923`.
...
The workflow attaches `ndu-or-r21/final-sha` to the actual final published commit after independent final-checkout replay, PDF and hash verification. A successful source checkpoint alone is not called a completed revision. Author approval of authorship, conflicts and submission declarations is still required. This branch is a referee-ready research package, not journal acceptance.
'''
 (ROOT/'README.md').write_text(intro)
 (ROOT/'NDU_OR_submission_checklist.md').write_text(f'''# Operations Research R21 submission checklist

## Current artifacts and journal format

- Current ordinary root R21 main source/PDF and electronic companion source/PDF exist and agree.
...
- Final published SHA, not merely an input-source SHA, must have `ndu-or-r21/final-sha` success before a clean final-checkout claim.
...
''')
 (R/'README.md').write_text('''# R21 reproduction and review guide
...
bash revisions/or-r21-20260923/run_all.sh
```
...
The initial publication source checkpoint takes the exact R19 snapshot and verifies `PREDECESSOR_SHA256.json`; this is not a completed manuscript claim. The workflow subsequently commits actual ordinary R21 manuscripts and data, then checks out that final SHA independently. A failed source checkpoint is not mislabeled a publication.

## Frozen final-checkout verification without recomputation or mutation

```sh
python -S revisions/or-r21-20260923/replay.py --check
python -S revisions/or-r19-20260923/replay.py --check
python revisions/or-r21-20260923/check_package.py --verify-manifest
```
...
''')
 prov={'revision':'R21','review_commit':'18381729edf8d15a536ebef16abf6660ee0db48f','complete_manuscript_base':'cb4f8644652ebe92f0aac5294baaff898d97f5cf',
   ...
   'final_sha_validation_context':'ndu-or-r21/final-sha','timing':'Actual recorded publication-host measurements; development pilots not substituted.',
 ...
 for directory in [R,ROOT/'revisions/or-r16-20260922',ROOT/'revisions/or-r19-20260923',ROOT/'reviews']:
  for p in directory.rglob('*'):
   if p.is_file() and '__pycache__' not in p.parts and p.name!='MANIFEST.json' and p.suffix not in ['.pid','.so'] and p.name not in ['run-all.log','execution.log']:
    paths.append(p)
 manifest={'revision':'R21','sha256':{str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted(set(paths))}}
 (R/'MANIFEST.json').write_text(json.dumps(manifest,indent=2)+'\n')
 print('R21 review metadata and manifest:',len(manifest['sha256']),'files')
