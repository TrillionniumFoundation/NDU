...
**Current revision:** R21. **Branch:** `revision/ndu-operations-research-r21-20260923`.
...
We agree with the version diagnosis. R20's plan-only tip is not called a completed manuscript. R21 starts from the review commit and preserves the exact R19 root files under `predecessor/`, with pinned SHA-256 values in `PREDECESSOR_SHA256.json`. It replaces both root manuscripts and their metadata with actual R21 artifacts. The complete source, derivations, raw records, generated tables and response accompany them. Publication and independent verification are separate workflow jobs; commit-status context `ndu-or-r21/final-sha` is attached to the final published SHA, not merely the input source SHA.

Locations: root `main.tex`, `main.pdf`, `electronic_companion.tex`, `electronic_companion.pdf`; `PROVENANCE.json`; `MANIFEST.json`; `results/package_check.json`; `.github/workflows/ndu-or-r21-publication.yml`.
...
Locations: abstract and introduction; main Sections `sec:r21-certification`, `sec:r21-benchmark` and the current computational study; `tables/matched.tex`; `matched_cost.py`; `results/matched_rows.json`.
...
Locations: `sections/literature.tex`, `sections/certification.tex`; main labels `thm:r21-decomposition`, `prop:r21-polishing`; `LITERATURE_MAP.md`.
...
Location: main `sec:r19-bridge` and `sec:r21-benchmark`; literature comparison table.
...
Locations: `validation.py`; main `prop:r21-validation`; `results/validation_summary.json`; independent reconstruction in `replay.py`.
...
Location: main `eq:r21-regularization`; `theory_checks.py` exact noncurved/nonsmooth examples; no regularity assumption is hidden in the generalized statement.
...
R21 implements and executes price polishing. For every matched record, original and repaired decisions/tensions/inequality prices must be identical, the rational upper bound cannot increase, and the final target must pass after any actual fallback. The independent replay checks those facts. This distinguishes an executed algorithmic addition from R20's unexecuted plan.
