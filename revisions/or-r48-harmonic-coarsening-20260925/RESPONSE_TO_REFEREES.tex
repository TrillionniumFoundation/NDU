...
\begin{document}
\begin{center}\Large\bfseries Response to the independent R45 referee report\end{center}


\textbf{Manuscript:} Limited-Memory Renewal Contracts: Harmonic Coarsening and Certified Joint Design

\textbf{Revision:} R48, September 25, 2026

\textbf{Report:} \nolinkurl{operation_research_referee_report_r45_independent_harsh_2026-09-25.md}

\textbf{Review commit:} 260a5224c55e0326d9da7f0c813b4a3fcab7671f

\textbf{Immediate scientific base:} R47, df1e192fd0846315ff0938e57d797efcd04109aa
...
\section*{1. Complexity frontier and the role of the full target grid}


\textbf{Referee concern: the only original-budget global guarantee is a direct grid whose exponent is the raw branch count.}
...
\texttt{1 + E (1 + ceil(2L/epsilon)) (1 + ceil(sqrt(2 Gamma/epsilon)))}.
...
\textbf{Locations:} Sections on exact types, two-sided decomposition, adaptive target boxes, certified response coarsening, and harmonic pooling; Theorems r46-box, r46-adaptive, r47-variable, and r48-scheme; the theorem-level provenance table.



\section*{2. What is new beyond classical interval dynamic programming}


\textbf{Referee concern: the conditional recurrence is useful but too modest as a centerpiece.}
...
\textbf{Locations:} Theorems r48-harmonic and r48-lift, Lemma r48-square, and the discussion of classical antecedents in the introduction.



\section*{3. Scalable guaranteed joint computation and stronger large-history certificates}


\textbf{Referee concern: the large experiments are conditional at ideal targets and do not test joint target choice.}
...
\textbf{Locations:} Joint computational evidence; Table r48-paired; Computational Reproduction Record, complete paired outcomes and raw certificate paths.



\section*{4. Testing the guaranteed method and separating heuristics}


\textbf{Referee concern: the target-net tests are tiny and the 35/36 and 36/36 statements describe a heuristic portfolio.}
...
\section*{5. Independent comparisons and numerical provenance}


\textbf{Referee concern: successful MIP containment alone is inadequate; report runtimes, nodes, errors, and limits.}
...
\textbf{Locations:} Computational Reproduction Record, independent numerical comparisons; PROTOCOL.json, results/mip.json, and certificate check outputs.



\section*{6. Inexact supports, local bounds, and policy recovery}


\textbf{Referee concern: an inexact endpoint routine must supply certified global bounds and a bracket; its compression penalty can be large.}
...
\section*{7. Operational interpretation and resource accounting}


\textbf{Referee concern: no independent application calibration is supplied, and command bits are not the entire implementation memory.}
...
\section*{8. Organization, length, and preservation}


\textbf{Referee concern: the cumulative breadth obscures the strongest joint result.}
