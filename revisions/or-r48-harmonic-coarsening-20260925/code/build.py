"""Build and audit R48 OR readers, without touching antecedent source files."""
from pathlib import Path
import re,json,subprocess,hashlib,shutil,sys,datetime,platform
from assemble import run as assemble,ABSTRACT
from tables import run as tables
R=Path(__file__).resolve().parents[1];ROOT=R.parents[1];B=ROOT/'.build/r48'
...
def build():
    ...
    for n,stub in [('main','main'),('electronic_companion','ec'),('COMPUTATIONAL_RECORD','cs')]:labels(n,ROOT/f'r48-{stub}-labels.aux')
    for _ in range(3):
        for n,stub in [('main','main'),('electronic_companion','ec'),('COMPUTATIONAL_RECORD','cs')]:
            ...
            tex(p);labels(n,ROOT/f'r48-{stub}-labels.aux')
    ...
    old=set()
    for no,name in [(45,'budgeted-compression-20260924'),(46,'box-decomposition-20260925'),(47,'joint-certificates-20260925')]:
    ...
    new=set(re.findall(pat,(R/'harmonic.tex').read_text()))
    inputs={}
    ...
    result=dict(status='PASS',utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),source_commit=commit,
        baseline='df1e192fd0846315ff0938e57d797efcd04109aa',pages=pages,main_nonreference_pages=nonrefs,
