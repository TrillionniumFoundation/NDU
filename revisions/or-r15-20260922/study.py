#!/usr/bin/env python3
"""Execute the preregistered repeated-train/independent-deployment R15 study."""
...
def run():
    ...
    summary={'revision':'R15','replications':8,'training_contexts_per_replication':256,'independent_deployment_contexts_per_replication':256,'methods':aggregate,'paired_neural_regret_difference':{'mean':float(np.mean(diff)),'standard_error':se,'interval95':[float(np.mean(diff)-margin),float(np.mean(diff)+margin)],'units':'independent training/deployment pair; approximate Student interval'},'reference_mean_gain':float(np.mean([c['reference_gain'] for c in allcohorts])),'timing':timing_summary,'offline':{'full_setup':s.full_setup,'previous_full_setup':s.previous_setup,'base_setup':s.base_setup,'mean_labels_seconds':float(np.mean([r['label_seconds'] for r in traindata])),'mean_fit_all_seconds':float(np.mean([r['fit_all_seconds'] for r in traindata]))},'frozen_bounds':{'M':M,'delta':.05,'n':256,'deterministic_gain_range_upper':bound,'exact_range_bound':str(exact_bound),'penalty':float(penalty),'bounds':frozen,'note':'A zero lower bound is vacuous beyond comparator safety; no population training guarantee is inferred.'},'certificate_records':records,'python':sys.version,'platform':platform.platform()}
