"""Build the R35 article, companion, and response and bind readers to exact sources."""
from pathlib import Path
import hashlib,json,os,re,subprocess,platform
ROOT=Path.cwd();R=ROOT/'revisions/or-r35-monge-frontier-20260923'
B=ROOT/'.build/ndu-r35';B.mkdir(parents=True,exist_ok=True)
...
assert 'Revision R35' in main and 'Revision R35' in ec
...
for f in ('r35-main-labels.aux','r35-ec-labels.aux'):(ROOT/f).write_text('')
for i in range(1,5):
 for doc,labels in [('main','r35-main-labels.aux'),('electronic_companion','r35-ec-labels.aux')]:
...
refs_first,refs_last=page('r35-refs-start'),page('r35-refs-end')
...
for phrase in ('Piecewise-Quadratic','Global Randomized Memory Frontier','Pathwise randomization','Monge Structure','R35'):
...
if not local:
 ...
 for line in changes:
  ...
  assert path in allowed or path.startswith(str(R.relative_to(ROOT))+'/') or path.startswith('.r35-transport/') or path in ('.github/workflows/ndu-or-r35-publication.yml','r35-main-labels.aux','r35-ec-labels.aux'),line
