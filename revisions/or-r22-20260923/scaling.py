...
def run():
          ...
          for name in names[shift:]+names[:shift]:
           for tol in tolset:
            ...
            try:
              ...
              if fallback:
                # Fresh workspace: no method inherits another method's refinement state.
    ...
    (OUT/'scaling_summary.json').write_text(json.dumps({'status':'PASS' if all(r['status']=='PASS' for r in rows) else 'FAILURES_RETAINED',
      'refinement_protocol':'Fresh lifted full-objective solve at numerical tolerance 1e-9 for every method; setup and final rational audit included in online time.','observations':len(rows),'label_records':64*len(specs),'summary':summary,'setup':setup,'platform':platform.platform(),
