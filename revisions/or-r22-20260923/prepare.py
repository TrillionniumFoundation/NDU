...
def inline(s):
    def f(m):
        ...
        return '\n% BEGIN R22 source: '+str(path.relative_to(ROOT))+'\n'+inline(path.read_text())+'\n% END R22 source\n'
...
def run():
    ...
    preserved=main[start:end];main=main[:start]+''.join((R/'sections'/n).read_text()+'\n' for n in ['certification.tex','complete_dual.tex','benchmark.tex','empirical.tex'])+main[end:]
    ...
    intro=('This electronic companion contains the new R22 protocol and the complete preserved R19 empirical section, followed by all earlier scalar, verified-cell, and implementation developments. Historical wording is dated explicitly and is not pooled as new evidence. The R22 predecessor directory preserves the complete R19 root manuscripts byte for byte; older revision directories and the historical supplement are unchanged. The R19 fleet calculation is retained under its stated fixed-policy assumptions; the current empirical confidence claims use only the freshly reset final validation described below.\n\n')
    ec=ec[:aa]+intro+(R/'sections/companion.tex').read_text()+'\n'+preserved+'\n'+ec[bb:]
    ec=ec.replace('\\input{revisions/or-r22-20260923/tables/geometry.tex}', '\\input{revisions/or-r22-20260923/tables/geometry.tex}\n\\input{revisions/or-r22-20260923/tables/setup.tex}')
    for name,s in [('main',main),('electronic_companion',ec)]:
        s=s.replace(OLDTITLE,TITLE).replace('Revision R19, September 23, 2026','Revision R22, September 23, 2026')
    ...
    print('Complete R22 main and companion sources assembled.')
